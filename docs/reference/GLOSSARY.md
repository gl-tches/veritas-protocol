...
### Epoch
A time period of 30 days used for message pruning. After an epoch ends, message bodies and ML-DSA signatures are pruned from the chain; only headers remain permanently. Pruning is deterministic — all nodes prune at the same boundary.
...
### Light Validator
A validator tier that holds headers and signatures only (no message bodies). Targets 256MB RAM. After epoch end, signatures are pruned and light validators converge to header-only state.
...
### Message Transaction
An on-chain transaction containing an encrypted message. Consists of a permanent header, an encrypted body (pruned after epoch), and an ML-DSA-65 signature (pruned after epoch).

### ML-DSA (Module-Lattice Digital Signature Algorithm)
A post-quantum digital signature algorithm (NIST FIPS 204). VERITAS uses ML-DSA-65 (security level 3) for all signing operations. Key sizes: public key 1,952 bytes, signature 3,309 bytes, private key 4,032 bytes.

### ML-KEM (Module-Lattice Key Encapsulation Mechanism)
A post-quantum key encapsulation algorithm (NIST FIPS 203). VERITAS uses ML-KEM-768 for key exchange.
...
### Padding
Adding extra bytes to messages to hide their true size. VERITAS pads to fixed buckets (1024, 2048, 4096, 8192 bytes).
...
### Reputation Score
A number (0-1000) representing a user's trustworthiness. Starts at 100. Asymmetric decay: above 500 decays toward 500, below 500 decays toward 0.
...
### Validator
A node that participates in blockchain consensus. Requires 700+ reputation. Two tiers: full validators (complete blocks) and light validators (headers only, 256MB RAM target).

### Validator Set
The group of active validators (max 21).

### Validator Trust Depth
The number of trust chain levels used for fallback validator discovery (3 in VERITAS).
...
| `MAX_ENVELOPE_SIZE` | 8,192 | Maximum envelope size in bytes |
| `MESSAGE_TTL` | 7 days | Message expiry time |
| `EPOCH_DURATION` | 30 days | Epoch duration for pruning |
...
| `REPUTATION_START` | 100 | Starting reputation score |
| `REPUTATION_MAX` | 1000 | Maximum reputation |
| `REPUTATION_QUARANTINE` | 200 | Quarantine threshold |
| `MIN_VALIDATOR_REPUTATION` | 700 | Minimum reputation to validate |
| `MAX_VALIDATORS` | 21 | Maximum active validators |
| `VALIDATOR_TRUST_DEPTH` | 3 | Lines of trust fallback |
| `PADDING_BUCKETS` | 1024, 2048, 4096, 8192 | Message padding sizes |
| `MAX_JITTER_MS` | 3000 | Maximum timing jitter |
| `ML_DSA_65_PK_SIZE` | 1,952 | ML-DSA-65 public key bytes |
| `ML_DSA_65_SIG_SIZE` | 3,309 | ML-DSA-65 signature bytes |
