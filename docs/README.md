# VERITAS Protocol Documentation

Complete documentation for the VERITAS Protocol.

**Version**: 0.4.0-beta | **Rust Edition**: 2024 | **MSRV**: 1.85 | **License**: MIT OR Apache-2.0

---

## Getting Started

- [Quickstart](getting-started/QUICKSTART.md) — Get running in 5 minutes
- [Installation](getting-started/INSTALLATION.md) — Detailed installation guide
- [Configuration](getting-started/CONFIGURATION.md) — Configuration reference

## Guides

- [Deployment](guides/DEPLOYMENT.md) — Production deployment guide
- [CLI Reference](guides/CLI_REFERENCE.md) — Command-line interface documentation
- [API Examples](guides/API_EXAMPLES.md) — Practical code examples
- [Development](guides/DEVELOPMENT.md) — Contributing to VERITAS
- [Troubleshooting](guides/TROUBLESHOOTING.md) — Common issues and solutions

## Reference

- [API](reference/API.md) — Complete API reference
- [Architecture](reference/ARCHITECTURE.md) — System architecture and design
- [Security](reference/SECURITY.md) — Security model and threat analysis
- [Glossary](reference/GLOSSARY.md) — Terms, concepts, and protocol constants
- [VERITAS-2026-0090 Remediation](reference/VERITAS-2026-0090-REMEDIATION.md) — Username uniqueness fix

## Additional Resources

- [Security Audit Report](../SECURITY_AUDIT_REPORT.md) — Completed January 2026 (90 issues fixed)
- [Version History](../VERSION_HISTORY.md) — Changelog
- [README](../README.md) — Project overview
- [CLAUDE.md](../CLAUDE.md) — Development instructions

---

## Directory Structure

```
docs/
├── README.md                      ← This file (documentation index)
├── getting-started/
│   ├── QUICKSTART.md              ← Quick start guide
│   ├── INSTALLATION.md            ← Full installation instructions
│   └── CONFIGURATION.md           ← Configuration options
├── guides/
│   ├── DEPLOYMENT.md              ← Production deployment
│   ├── CLI_REFERENCE.md           ← CLI commands reference
│   ├── API_EXAMPLES.md            ← Code examples
│   ├── DEVELOPMENT.md             ← Development guide
│   └── TROUBLESHOOTING.md         ← Problem solving
└── reference/
    ├── API.md                     ← API documentation
    ├── ARCHITECTURE.md            ← System design
    ├── SECURITY.md                ← Security documentation
    ├── GLOSSARY.md                ← Terminology
    └── VERITAS-2026-0090-REMEDIATION.md  ← Security fix details
```
